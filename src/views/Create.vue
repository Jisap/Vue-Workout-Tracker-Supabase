...
export default {
  ...
  setup() {
    ...
    // Add exercise
    const addExercise = () => {
      if (workoutType.value === "strength") { // Si el entrenamiento es fuerza
        exercises.value.push({                // enviamos a exercises[] los inputs del formulario
          id: uid(),
          exercise: "",
          sets: "",
          reps: "",
      ...
      exercises.value.push({    // Si es cardio la estructura del formulario es diferente
        id: uid(),
        cardioType: "",
        distance: "",
        duration: "",
    ...
    const deleteExercise = (id) => {
      if (exercises.value.length > 1) {                                             // Si exercises[] tiene almenos 1 elemento
        exercises.value = exercises.value.filter((exercise) => exercise.id !== id); // filtramos el exercise cuyo id venga en el argumento 
        return;
      }
    ...
    const createWorkout = async () => {
      try {
        const { error } = await supabase.from("workouts").insert([  // Insertamos en supabase los valores del formulario
          {
        ...
        if (error) throw error;                                     // Si hubo algún error lanzamos el error
        statusMsg.value = "Succes: Workout Created!";               // Sino lo hubo mensaje de éxito
        workoutName.value = null;                                   // y reiniciamos los valores del formulario
